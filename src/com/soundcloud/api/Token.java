...
public class Token implements Serializable {
        ...
        try {
            for (Iterator it = json.keys(); it.hasNext();) {
                ...
                if (ACCESS_TOKEN.equals(key)) {
                    ...
                    expiresIn = System.currentTimeMillis() + json.getLong(key)
                            * 1000;
    ...
    public boolean valid() {
        return access != null
                && (scoped(SCOPE_NON_EXPIRING) || refresh != null);
    }
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof Token) {
            Token token = (Token) o;
            if (access != null ? !access.equals(token.access)
                    : token.access != null)
                return false;
            if (refresh != null ? !refresh.equals(token.refresh)
                    : token.refresh != null)
                return false;
            if (scope != null ? !scope.equals(token.scope)
                    : token.scope != null)
                return false;
            return true;
        } else {
