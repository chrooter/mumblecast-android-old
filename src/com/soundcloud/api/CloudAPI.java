...
public interface CloudAPI {
    ...
    String FACEBOOK_GRANT_TYPE = "urn:soundcloud:oauth2:grant-type:facebook&access_token="; // oauth2
                                                                                            // extension
    ...
    /**
     * Request a token using <a
     * href="http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.1.2">
     ...
     *            SoundCloud password
     * @param scopes
     *            the desired scope(s), or empty for default scope
     ...
     */
    Token login(String username, String password, String... scopes)
            throws IOException;

    /**
     * Request a token using <a
     * href="http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-4.1.1">
    ...
    /**
     * Request a "signup" token using <a
     * href="http://tools.ietf.org/html/draft-ietf-oauth-v2-15#section-4.4">
     * Client Credentials</a>.
     * 
     * Note that this token is <b>not</b> set as the current token in the
     * wrapper - it should only be used for one request (typically the signup /
     * user creation request). Also note that not all apps are allowed to
     * request this token type (the wrapper throws InvalidTokenException in this
     * case).
     * 
     * @param scopes
     *            the desired scope(s), or empty for default scope
     ...
     *             IO/Error
     * @throws com.soundcloud.api.CloudAPI.InvalidTokenException
     *             if requested scope is not available
     */
    Token clientCredentials(String... scopes) throws IOException;

    /**
     * Request a token using an <a
     * href="http://tools.ietf.org/html/draft-ietf-oauth-v2-22#section-4.5">
     ...
     * @throws IOException
     */
    Token extensionGrantType(String grantType, String... scopes)
            throws IOException;
    ...
    /**
     * This method should be called when the token was found to be invalid. Also
     * replaces the current token, if there is one available.
     * 
     * @return an alternative token, or null if none available (which indicates
     *         that a refresh could be tried)
     */
    ...
    /**
     * Generic execute method, with added workarounds for various HTTPClient
     * bugs.
     * 
     * @param target
     *            the target host (can be null)
     * @param request
     *            the request
     * @return the HTTP response
     * @throws IOException
     *             network errors
     * @throws BrokenHttpClientException
     *             in case of HTTPClient framework bugs
     */
    HttpResponse safeExecute(HttpHost target, HttpUriRequest request)
            throws IOException;
     ...
     * 
     * @param uri
     *            SoundCloud model URI, e.g. http://soundcloud.com/bob
     * @return the id
     * @throws IOException
     *             network errors
     * @throws ResolverException
     *             if object could not be resolved
     */
     ...
     * 
     * @param uri
     *            SoundCloud stream URI, e.g.
     *            https://api.soundcloud.com/tracks/25272620/stream
     * @param skipLogging
     *            skip logging the play of this track (client needs
     *            {@link com.soundcloud.api.Token#SCOPE_PLAYCOUNT})
     * @return the resolved stream
     * @throws IOException
     *             network errors
     * @throws com.soundcloud.api.CloudAPI.ResolverException
     *             resolver error (invalid status etc)
     */
    ...
    /**
     * Registers a listener. The listener will be informed when an access token
     * was found to be invalid, and when the token had to be refreshed.
     * 
     * @param listener
     *            token listener
     */
    ...
    /**
     * Request login via authorization code After login, control will go to the
     * redirect URI (wrapper specific), with one of the following query
     * parameters appended:
     * <ul>
     * <li><code>code</code> in case of success, this will contain the code used
     * for the <code>authorizationCode</code> call to obtain the access token.
     * <li><code>error</code> in case of failure, this contains an error code
     * (most likely <code>access_denied</code>).
     * </ul>
     * 
     * @param options
     *            auth endpoint to use (leave out for default), requested scope
     *            (leave out for default), display ('popup' for mobile optimized
     *            screen) and state.
    ...
    /**
     * Changes the default content type sent in the "Accept" header. If you
     * don't set this it defaults to "application/json".
     * 
     * @param contentType
    ...
    class InvalidTokenException extends IOException {
         ...
         *            the HTTP error code
         * @param status
         *            the HTTP status, or other error message
         */
    ...
    /**
     * Thrown if the service API responds in error. The HTTP status code can be
     * obtained via {@link #getStatusCode()}.
     */
