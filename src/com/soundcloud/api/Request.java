...
public class Request implements Iterable<NameValuePair> {
    ...
    private List<NameValuePair> mParams = new ArrayList<NameValuePair>(); // XXX
                                                                          // should
                                                                          // probably
                                                                          // be
                                                                          // lazy
    ...
    public Request(String resource) {
        ...
        if (resource.contains("?")) {
            String query = resource.substring(
                    Math.min(resource.length(), resource.indexOf("?") + 1),
                    resource.length());
            for (String s : query.split("&")) {
                ...
                if (kv != null) {
                    try {
                        if (kv.length == 2) {
                            mParams.add(new BasicNameValuePair(URLDecoder
                                    .decode(kv[0], UTF_8), URLDecoder.decode(
                                    kv[1], UTF_8)));
                        } else if (kv.length == 1) {
                            mParams.add(new BasicNameValuePair(URLDecoder
                                    .decode(kv[0], UTF_8), null));
                        }
     ...
     * 
     * <pre>
     * Request r = new Request.to(&quot;/foo&quot;).add(&quot;singleParam&quot;, &quot;value&quot;)
    ...
    private void addParam(String name, Object param) {
        mParams.add(new BasicNameValuePair(name, param == null ? null : String
                .valueOf(param)));
    }
     ...
     * @param resource
     *            the new resource
     * @return a new request with identical parameters except for the specified
     *         resource.
     */
     ...
     *            the file to be submitted
     * @param fileName
     *            the name of the uploaded file (over rides file parameter)
     * @return this
     */
     ...
     *            the data to be submitted
     * @return this
     * @deprecated use {@link #withFile(String, java.nio.ByteBuffer), String}
     *             instead
     */
    @Deprecated
     ...
     * 
     * @param etag
     *            the etag to check for (If-None-Match: etag)
     * @return this
     */
            ...
            if (request instanceof HttpEntityEnclosingRequestBase) {
                ...
                if (isMultipart()) {
                    MultipartEntity multiPart = new MultipartEntity(
                            HttpMultipartMode.BROWSER_COMPATIBLE, // XXX change
                                                                  // this to
                                                                  // STRICT once
                                                                  // rack on
                                                                  // server is
                    ...
                    if (mFiles != null) {
                        for (Map.Entry<String, Attachment> e : mFiles
                                .entrySet()) {
                            multiPart.addPart(e.getKey(), e.getValue()
                                    .toContentBody());
                        }
                    }
                    ...
                    for (NameValuePair pair : mParams) {
                        multiPart.addPart(pair.getName(),
                                new StringBody(pair.getValue(), "text/plain",
                                        charSet));
                    }
                    ...
                    if (!mParams.isEmpty()) {
                        ...
                        enclosingRequest.setEntity(new StringEntity(
                                queryString()));
                    }
                    request.setURI(URI.create(mResource));
                }
    ...
    public String toString() {
        return "Request{" + "mResource='" + mResource + '\'' + ", params="
                + mParams + ", files=" + mFiles + ", entity=" + mEntity
                + ", mToken=" + mToken + ", listener=" + listener + '}';
    }
    ...
    public static interface TransferProgressListener {
         ...
         * @throws IOException
         *             if the transfer should be cancelled
         */
